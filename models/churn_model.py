...
def load_or_train_model() -> Tuple[LogisticRegression, Optional[StandardScaler]]:
    ...
    if MODEL_PATH.exists():
        ...
        logger.info("Loaded REAL churn model | path=%s", MODEL_PATH)
        if scaler:
            logger.info("Loaded churn scaler | path=%s", SCALER_PATH)
        else:
            logger.warning("No scaler found — using raw 0-1 normalization")
        return model, scaler
    ...
    logger.warning("FALLBACK model saved to %s. Run 'python -m data.train' to train on real data.", MODEL_PATH)
    return model, None
